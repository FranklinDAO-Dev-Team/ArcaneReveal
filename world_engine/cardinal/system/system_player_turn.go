...
func PlayerTurnSystem(world cardinal.WorldContext) error {
	return cardinal.EachMessage[msg.PlayerTurnMsg, msg.PlayerTurnResult](
		...
		func(turn message.TxData[msg.PlayerTurnMsg]) (msg.PlayerTurnResult, error) {
			...
			if player == nil {
				return msg.PlayerTurnResult{}, fmt.Errorf("Player Not Found")
			}

			fmt.Printf("nickname: %s, turn.Msg.Action: %s,, action: %s", player.Nickname, turn.Msg.Action, turn.Msg.Action)
			...
			case "move":
				player_turn_move(world, player, turn.Msg.Direction)
			default:
				return msg.PlayerTurnResult{}, fmt.Errorf("Invalid action")
			}
